...
import com.marcel.malewski.ticketsale.seat.SeatRepository;
import com.marcel.malewski.ticketsale.ticket.dto.TicketResponseDto;
import com.marcel.malewski.ticketsale.ticket.exceptions.TicketNotFoundException;
import com.marcel.malewski.ticketsale.ticketbuyer.TicketBuyer;
import com.marcel.malewski.ticketsale.ticketbuyer.TicketBuyerRepository;
...
public class TicketService {
   ...
   public TicketWithValidation getTicketWithValidationById(long id) {
      Ticket ticket = ticketRepository.findById(id).orElseThrow(
              () -> new TicketNotFoundException(TicketConstants.TICKET_BY_ID_NOT_FOUND_MESSAGE + id));
...
//   }
   ...
   public void putTicketById(long id, TicketWithValidation ticketWithValidation) {
      Ticket ticket = this.ticketRepository.findById(id).orElseThrow(
              () -> new TicketNotFoundException(String.format(TicketConstants.TICKET_BY_ID_NOT_FOUND_MESSAGE, id)));
...
//   }
   ...
   public void deleteTicketById(long id) {
      if (!this.ticketRepository.existsById(id))
         throw new TicketNotFoundException(String.format(TicketConstants.TICKET_BY_ID_NOT_FOUND_MESSAGE, id));
